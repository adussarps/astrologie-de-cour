...
import {
  ...
  enSigne, dateCivile, PLANETES, SIGNES,
} from './ciel.js';
import {
...
/** L'époque Unix en jour julien, et l'instant présent. */
const JJ_EPOQUE = 2440587.5;
const maintenantEnJJ = () => Date.now() / 86400000 + JJ_EPOQUE;

/** La date et l'heure locales d'un instant, dans le fuseau d'un lieu. */
function dateLocales(jj, latitude, longitude) {
  const zone = fuseauDe(latitude, longitude) ?? 'UTC';
  const quand = new Date((jj - JJ_EPOQUE) * 86400000);
  const parties = Object.fromEntries(new Intl.DateTimeFormat('fr-FR', {
    timeZone: zone, year: 'numeric', month: '2-digit', day: '2-digit',
    hour: '2-digit', minute: '2-digit', hourCycle: 'h23',
  }).formatToParts(quand).map((p) => [p.type, p.value]));
  return {
    zone,
    annee: +parties.year, mois: +parties.month, jour: +parties.day,
    heure: +parties.hour, minute: +parties.minute,
  };
}

/** Tout ce qu'il faut pour afficher une figure.
 *
 *  `jjImpose` court-circuite la convention de temps : c'est ce qu'il faut pour
 *  un instant que l'on connaît déjà exactement, comme « maintenant ». Sans lui,
 *  l'heure saisie est interprétée selon la convention choisie. */
function dresser(saisie, jjImpose = null) {
  const base = jjImpose === null ? instant(saisie) : {
    jj: jjImpose, julien: false, convention: null,
    zone: fuseauDe(saisie.latitude, saisie.longitude), decalage: null, equation: 0,
  };
  const { jj, julien, convention, decalage, equation, zone } = base;
...
function rendreAnnee(saisie, age) {
  ...
  const julien = estJulien(saisie);
  const d = dateCivile(f.jj, julien);
  const fin = dateCivile(f.finit, julien);
  const m = f.maitre;
  ...
  return `<div class="issue">
    ...
    : 'Non — il lui manque de quoi tenir sa promesse'}</dd>
      ...
      ${actions('annee', dossierAnnee({ saisie, resultat, annee: f, julien }))}
    </div>
  </div>
  <details class="detail">
...
function blocMois(f, julien) {
  const lignes = f.mois.map((m) => {
    const d = dateCivile(m.debut, julien);
...
function rendreQuestion(rangMaison, texteQuestion) {
  const latitude = +$('#latitude').value || 48.8566;
  const longitude = +$('#longitude').value || 2.3522;
  const lieu = $('#lieu').value.trim() || 'Paris';
  // « Maintenant » est un instant exact : on ne le fait passer par aucune
  // convention d'heure, et le lieu est celui du formulaire, non Paris en dur.
